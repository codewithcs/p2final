...
// function and variable names cannot be same 
 ...
 int nextRouterPort() const { return (Dead() ? -1 : nextrouterport  ); }
 char nextRouterName() const { return (Dead() ? '0' : nextroutername  ); }
 int Cost() const { return (Dead() ? -1 : cost); }
 bool Dead() const { return dead; }

 void setNextRouterPort(int n) { nextrouterport = n; }
 void setNextRouterName(char n) { nextroutername = n; }
...
private:
 bool dead;
 int nextrouterport; // port number of next hop router
 char nextroutername;
